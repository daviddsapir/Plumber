...
Menu::Menu() :
	...
	exitGameBtn(sf::Vector2f(WINDOW_WIDTH * 0.5,
		WINDOW_HEIGHT * 0.1)),
	aboutBtn(sf::Vector2f(WINDOW_WIDTH * 0.5,
		WINDOW_HEIGHT * 0.1))
{
	// Set the screen menu
	setMenuScreen();
	...
	sf::RenderWindow& window)
{
	...
	if (aboutBtn.getGlobalBounds().contains(location))
		aboutBtn.setTexture(Resources::instance().
			getWindowObjectTexture(BUTTON_HOVER));
	else
		aboutBtn.setTexture(Resources::instance().
			getWindowObjectTexture(BUTTON));
	...
	bool& showInstruction)
{
	...
	// Handle the event the mouse click is released upon the 
	// Instructions button
	else if (aboutBtn.getGlobalBounds().contains(location))
	{
...
void Menu::setMenuScreen()
{
	...
	setButton(aboutBtn, instructionTxt, "About",
		WINDOW_WIDTH * 0.45, WINDOW_HEIGHT * 0.41);

	// Set the Exit button with the sended args
	setButton(exitGameBtn, exitTxt, "Exit",
		WINDOW_WIDTH * 0.46, WINDOW_HEIGHT * 0.61);
}
	...
	const int width, const int height)
{
	...
	// set the text style
	buttonText.setStyle(TEXT_STYLE);
